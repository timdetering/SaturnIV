    ...
    public class RenderStarfield : Microsoft.Xna.Framework.GameComponent
    {
        ...
        private void generateStarField(Game game, int numberofStars)
        {
            ...
            for (int count = 0; count < numberofStars; count++)
            {
                ...
                x = w * Math.Cos(t);
                y = w * Math.Sin(t);
                starList[count] = new VertexPositionColor(new Vector3((float)x * rand.Next(-150000, 150000),
                                                        (float)y * rand.Next(-150000, 150000), (float)z * rand.Next(-150000, 150000)),
