...
ExpVertexToPixel ExplosionVS(float3 inPos: POSITION0, float4 inTexCoord: TEXCOORD0, float4 inExtra: TEXCOORD1)
{
	...
	float relAge = age/maxAge * 0.50f;	
	
	float sizer = saturate(1-relAge*relAge/1.5f);
	float size = 150.0f*random*sizer;
	...
	float alpha = 1.5-relAge*relAge;
	Output.Color = float4(0.7f,0.7f,0.7f,alpha);	
